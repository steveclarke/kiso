...
| Shared JS utils | Use `positionBelow()` from `utils/positioning.js`, `highlightItem()`/`wrapIndex()` from `utils/highlight.js`, and `FOCUSABLE_SELECTOR` from `utils/focusable.js`. Never reimplement positioning, list navigation, or focusable element queries. |
